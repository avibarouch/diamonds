...
# זה העתק של קובץ שנמצא בתקיית הנתונים (דטה)
# צריך לתקן אותו כך שיבנה פיקל ולא יותר
# לאחר מכן, יש להסיר את הכיתוב בעברית כמובן
# באחריות אנדי
...
def build_it():
    ...
    df00_0 = df1.query("(x < 0.1) & (y < 0.1) & (z < 0.1)")
    for i in range(3):
        ...
        df00_0 = pd.concat([df00_0, df00])
    df3 = df1.drop(axis=0, index=df00_0.index)
    ...
    X_train, X_test, y_train, y_test = \
        train_test_split(X, y, test_size=0.2, random_state=66)
